...
//-----------------------------------------------------------
   ...
   //printf( "Luminosity :%d + %d = %d\n", *((uint32_t *)h2p_lw_reg1_addr), *((uint32_t *)h2p_lw_reg2_addr));
   //printf( "Luminosity :%u + %u\n", (uint8_t)h2p_lw_reg1_addr, (uint8_t)h2p_lw_reg2_addr);
   //uint16_t combo = ( uint16_t )( ( ( *( uint16_t * )h2p_lw_reg1_addr ) <<8 ) + ( *(uint16_t *)h2p_lw_reg2_addr ) );
   //printf("%x +%x = %d lux \n", *(uint8_t *)h2p_lw_reg1_addr, *(uint8_t *)h2p_lw_reg2_addr, combo);
   printf("Address reg 1: %p.\n", h2p_lw_reg1_addr);
   printf("Address reg 2: %p.\n", h2p_lw_reg2_addr);
   printf("Value reg 1 [MSB]: %u.\n", *((uint8_t *)h2p_lw_reg1_addr));
   printf("Value reg 2 [LSB]: %u.\n", *((uint8_t *)h2p_lw_reg2_addr));
